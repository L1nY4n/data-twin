...
describe('simulation batch updates', () => {
  ...
  test('throttles movement-only runtime publishes while keeping ecs snapshots fresh', () => {
    const store = useDigitalTwinStore.getState()
    store.reset()

    store.addEntity({
      id: 'vehicle-live-throttle',
      type: 'vehicle',
      name: '实时推送车辆',
      position: { x: 0, y: 0, z: 0 },
      rotation: { x: 0, y: 0, z: 0 },
    ...
    const initialSnapshot = store.getEcsSnapshotById('vehicle-live-throttle')
    if (!initialSnapshot) throw new Error('snapshot missing')
    initialSnapshot.labelMode = 'hidden'
    store.updateEntity('vehicle-live-throttle', {})
    ...
    const afterFirstPublish = useDigitalTwinStore.getState()
    const beforeEntities = afterFirstPublish.entities
    const beforeBuckets = afterFirstPublish.entityBuckets
    let commits = 0
    const unsubscribe = useDigitalTwinStore.subscribe(() => {
      commits += 1
    })
    ...
    const afterSecondTick = useDigitalTwinStore.getState()
    const snapshot = afterSecondTick.getEcsSnapshotById('vehicle-live-throttle')

    expect(afterSecondTick.entities).toBe(beforeEntities)
    expect(afterSecondTick.entityBuckets).toBe(beforeBuckets)
    expect(commits).toBe(0)
    expect(snapshot?.position).toEqual({ x: 2, y: 0, z: 2 })
  })
})
