...
import { getVehicleSeparationDistance } from './vehicle-footprint'
  ...
  test('production profile does not spawn vehicles inside the vehicle safety radius', () => {
    ...
    for (let i = 0; i < vehicles.length; i += 1) {
      for (let j = i + 1; j < vehicles.length; j += 1) {
        const requiredDistance = getVehicleSeparationDistance(
...
describe('dynamic movement separation', () => {
  test('lets vehicle creep forward to the nearest safe following distance', () => {
    const result = applyDynamicSeparation(
      ...
      { x: 0, y: 0, z: 0 },
      { x: 2.4, y: 0, z: 0 },
      [{ id: 'vehicle-b', type: 'vehicle', vehicleType: 'car', position: { x: 5.4, y: 0, z: 0 } }],
      'car'
    )
  ...
  test('holds vehicle when there is no safe forward progress available', () => {
    const result = applyDynamicSeparation(
      ...
      { x: 0, y: 0, z: 0 },
      { x: 0.6, y: 0, z: 0 },
      [{ id: 'vehicle-b', type: 'vehicle', vehicleType: 'car', position: { x: 4.6, y: 0, z: 0 } }],
      'car'
    )
  ...
  test('holds person when the next step would enter vehicle safety radius', () => {
    const result = applyDynamicSeparation(
      ...
      { x: 0, y: 0, z: 0 },
      { x: 1.1, y: 0, z: 0 },
      [{ id: 'vehicle-b', type: 'vehicle', vehicleType: 'car', position: { x: 3.4, y: 0, z: 0 } }]
    )
  ...
  test('uses rendered vehicle footprints when separating truck and forklift traffic', () => {
    const requiredDistance = getVehicleSeparationDistance('forklift', 'truck')
    const result = applyDynamicSeparation(
      'forklift-a',
      'vehicle',
      { x: 0, y: 0, z: 0 },
      { x: 0.7, y: 0, z: 0 },
      [{ id: 'truck-b', type: 'vehicle', vehicleType: 'truck', position: { x: 6.8, y: 0, z: 0 } }],
      'forklift'
    )
  ...
  test('keeps vehicle type in occupancy buckets and queries the full footprint radius', () => {
    const requiredDistance = getVehicleSeparationDistance('truck', 'truck')
    const index = createDynamicOccupancyIndex([
      { id: 'truck-b', type: 'vehicle', vehicleType: 'truck', position: { x: 0, y: 0, z: 0 } },
    ])

    const occupants = queryDynamicOccupants(
      index,
      { x: requiredDistance - 0.1, y: 0, z: 0 },
      DYNAMIC_NEIGHBOR_QUERY_RADIUS,
      'truck-a'
    )

    expect(DYNAMIC_NEIGHBOR_QUERY_RADIUS).toBeGreaterThanOrEqual(requiredDistance)
    expect(occupants).toHaveLength(1)
    expect(occupants[0].vehicleType).toBe('truck')
  })

  test('can reuse the neighbor query result array on the simulation hot path', () => {
    const index = createDynamicOccupancyIndex([
      { id: 'vehicle-a', type: 'vehicle', vehicleType: 'car', position: { x: 0, y: 0, z: 0 } },
      { id: 'person-b', type: 'person', position: { x: 1, y: 0, z: 1 } },
    ])
    const scratch = [{ id: 'stale', type: 'person', position: { x: 99, y: 0, z: 99 } }]

    const first = queryDynamicOccupants(
      index,
      { x: 0, y: 0, z: 0 },
      DYNAMIC_NEIGHBOR_QUERY_RADIUS,
      'vehicle-a',
    ...
    const second = queryDynamicOccupants(
      index,
      { x: 80, y: 0, z: 80 },
      DYNAMIC_NEIGHBOR_QUERY_RADIUS,
      undefined,
