...
describe('admin management guards', () => {
  test('admin should use sidebar shell and route sections instead of a single tabs page', () => {
    ...
    const sharedSidebar = readFileSync(join(process.cwd(), 'components/ui/sidebar.tsx'), 'utf8')
    ...
    expect(adminMeta.includes('ADMIN_PAGE_REGISTRATIONS')).toBe(true)
    expect(adminMeta.includes('buildAdminNavGroups')).toBe(true)
    expect(adminMeta.includes('registration.showInNav === false')).toBe(true)
    expect(adminMeta.includes('BUILT_IN_ADMIN_PAGE_REGISTRATIONS')).toBe(true)
    ...
    expect(sidebar.includes('SidebarRail')).toBe(true)
    expect(sharedSidebar.includes('cursor-pointer')).toBe(true)
    expect(sharedSidebar.includes('cursor-w-resize')).toBe(false)
    expect(sharedSidebar.includes('cursor-e-resize')).toBe(false)
  ...
  test('admin console should load overview, governance and structured editor flows', () => {
    ...
    expect(consoleSource.includes('getAdminPageRegistration')).toBe(true)
    expect(consoleSource.includes('const builtInRenderers')).toBe(true)
    expect(consoleSource.includes('ModulePageHost')).toBe(true)
    ...
    expect(consoleSource.includes("case 'workspaces'")).toBe(false)
    expect(consoleSource.includes("case 'archetypes'")).toBe(false)
