...
describe('renderer backend guards', () => {
  ...
  test('canvas should use dual backend renderer creation helper', () => {
    ...
    expect(source.includes('RendererErrorBoundary')).toBe(true)
    expect(source.includes('data-renderer-error="true"')).toBe(true)
    expect(source.includes("fallbackReason: 'renderer-init-failed'")).toBe(true)
    ...
    expect(renderer.includes('WEBGPU_INIT_TIMEOUT_MS')).toBe(true)
    expect(renderer.includes('withRendererTimeout')).toBe(true)
    expect(renderer.includes("options.mode === 'webgpu'")).toBe(true)
    expect(source.includes('WEBGPU_FRAME_STALL_FALLBACK_MS')).toBe(true)
    expect(source.includes("rendererMode === 'webgpu' && backend !== 'webgpu'")).toBe(true)
    expect(source.includes("setRendererMode('webgl2')")).toBe(true)
  })
