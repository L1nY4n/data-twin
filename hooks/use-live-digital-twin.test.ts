...
describe('useLiveDigitalTwin live-only behavior', () => {
  ...
  test('decodes dense binary pose frames into pose buffers before throttled store sync', () => {
    ...
    expect(source.includes('poseBufferUpdates.push({ entityId, samples })')).toBe(true)
    expect(source.includes('runtimeVehiclePoseBuffer.upsertMany(poseBufferUpdates)')).toBe(true)
